...
WATER_VISCOSITY = 0.0010016          #(Pa.s = kg/(m.s))
...
def kpa_to_psi(pressure_kpa):
    ...
    return pressure_kpa *  0.145038 #1 kPa ~ 0.0145038 psi

def main():
    height = water_column_height(20, 5)
    pressure_kpa = pressure_gain_from_water_height(height)
    pressure_psi = kpa_to_psi(pressure_kpa)

    print(f"Final pressure: {pressure_kpa:.2f} kPa ({pressure_psi:.2f})")
