...
def test_kpa_to_psi():
    assert kpa_to_psi(0) == pytest.approx(0, rel=1e-5) 
    assert kpa_to_psi(1) == pytest.approx(0.145038, rel=1e-5) 
    assert kpa_to_psi(100) == pytest.approx(14.5038, rel=1e-4) 
    assert kpa_to_psi(101.325) == pytest.approx(14.696, rel=1e-3) # ~1 atm 
    assert kpa_to_psi(500) == pytest.approx(72.519, rel=1e-3)
